...
# create vpc module
module "vpc" {
  source                       = "git@github.com:nitheshsivakumar/terraform-modules.git//vpc"
  region                       = local.region
...
# create nat-gateways
module "nat_gateway" {
  source                     = "git@github.com:nitheshsivakumar/terraform-modules.git//nat-gateway"
  project_name               = local.project_name
  environment                = local.environment
...
module "security_group" {
  source       = "git@github.com:nitheshsivakumar/terraform-modules.git//security-groups"
  project_name = local.project_name
  environment  = local.environment
  vpc_id       = module.vpc.vpc_id
  ssh_ip       = [var.ssh_ip]
...
module "application_load_balancer" {
  source                = "git@github.com:nitheshsivakumar/terraform-modules.git//alb"
  project_name          = local.project_name
  environment           = local.environment
  ...
  vpc_id                = module.vpc.vpc_id
  certificate_arn       = module.ss_certificate.certificate_arn

}

# create s3 bucket
module "s3_bucket" {
  source               = "git@github.com:nitheshsivakumar/terraform-modules.git//s3"
  project_name         = local.project_name
...
module "ecs_task_execution_role" {
  source               = "git@github.com:nitheshsivakumar/terraform-modules.git//iam-role"
  project_name         = local.project_name
...
module "ecs" {
  source                       = "git@github.com:nitheshsivakumar/terraform-modules.git//ecs"
  project_name                 = local.project_name
  environment                  = local.environment
...
module "route_53" {
  source                             = "git@github.com:nitheshsivakumar/terraform-modules.git//route-53"
  domain_name                        = module.ss_certificate.domain_name
